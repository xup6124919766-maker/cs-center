 ...
 */
// ─── 推 CSAT 評分問卷（5 個 quick reply）─────────────────────────
export const sendCSATSurvey = async (accessToken, to, conversationId, clientId = null) => {
  const endpoint = `${LINE_API_BASE}/message/push`;
  const items = [1, 2, 3, 4, 5].map(s => ({
    type: 'action',
    action: {
      type: 'postback',
      label: '⭐'.repeat(s) + ` ${s}`,
      data: `csat=${s}&conv=${conversationId}`,
      displayText: `${s} 顆星`,
    },
  }));
  const reqBody = {
    ...
    messages: [{
      type: 'text',
      text: '感謝您剛才的詢問 🌙\n方便給我們這次服務 1-5 顆星嗎？您的回饋會讓我們做得更好。',
      quickReply: { items },
    }],
  };
  const start = Date.now();
  ...
  try {
    ...
    resBody = await res.json().catch(() => ({}));
    logApiCall({
      direction: 'outbound', channel: 'line', endpoint, method: 'POST',
      req_body: reqBody, res_status: resStatus, res_body: resBody,
      duration_ms: Date.now() - start, client_id: clientId,
    });
    return { ok: res.ok, status: resStatus, body: resBody };
  } catch (e) {
    logApiCall({ direction: 'outbound', channel: 'line', endpoint, method: 'POST',
      res_status: null, error: e.message, duration_ms: Date.now() - start, client_id: clientId });
    return { ok: false, error: e.message };
  }
};

// ─── 推圖片訊息 ───────────────────────────────────────────
// originalUrl 與 previewUrl 必須是公開的 https URL（≤ 10 MB / ≤ 240×240 預覽）
export const sendImage = async (accessToken, to, originalUrl, previewUrl, clientId = null) => {
  const endpoint = `${LINE_API_BASE}/message/push`;
  const reqBody = {
    to,
    messages: [{
  ...
  try {
    ...
    if (!res.ok) throw new Error(`LINE sendImage 失敗: ${resStatus} ${JSON.stringify(resBody)}`);
    return { ok: true };
  } catch (e) {
    if (resStatus === null) {
      logApiCall({ direction: 'outbound', channel: 'line', endpoint, method: 'POST',
