...
export const fetchAccessToken = async (email, password, type = 'store', baseUrl = DEFAULT_BASE) => {
  ...
  try {
    ...
    logApiCall({
      client_id: null, channel: 'bvshop', method: 'POST',
      endpoint: '/api/v2/get-token', res_status: resp.status,
    ...
    logApiCall({
      client_id: null, channel: 'bvshop', method: 'POST',
      endpoint: '/api/v2/get-token', res_status: 0,
      duration_ms: Date.now() - startAt, res_body: err.message,
    });
...
const apiGet = async (client, path) => {
  ...
  const doFetch = async (tk) => {
    ...
    logApiCall({
      client_id: client.id, channel: 'bvshop', method: 'GET',
      endpoint: path.split('?')[0], res_status: resp.status,
      duration_ms: Date.now() - startAt,
    });
    return resp;
  };
...
// ─── 用 phone/email/line userid 搜尋 BV 顧客 ───
// query 是任意字串：純數字當電話試 + 含 @ 當 email + 否則 line userid
// 回傳合併 dedupe 結果
export const searchBvCustomers = async (client, query) => {
  const q = String(query || '').trim();
  if (!q) return { ok: false, error: '空查詢' };

  const tasks = [];
  // email 含 @
  if (q.includes('@')) tasks.push({ key: 'email', val: q });
  // 8-15 位純數字 → 當電話
  if (/^[0-9]{6,15}$/.test(q)) tasks.push({ key: 'phone', val: q });
  // 否則當 line userid
  if (!q.includes('@')) tasks.push({ key: 'line', val: q });

  const seen = new Map();
  for (const t of tasks) {
    const r = await apiGet(client, `/api/v2/customers?${t.key}=${encodeURIComponent(t.val)}&limit=10`);
    if (!r.ok) continue;
    for (const c of (r.data?.data || [])) {
      if (!seen.has(c.id)) seen.set(c.id, c);
    }
  }
  return { ok: true, customers: Array.from(seen.values()) };
};

// ─── 取單筆 BV 顧客 ───
export const fetchCustomer = async (client, bvCustomerId) => {
  const r = await apiGet(client, `/api/v2/customers/${bvCustomerId}`);
  if (!r.ok) return { ok: false, error: r.error };
  return { ok: true, customer: r.data?.data || r.data };
};
...
const apiWrite = async (client, method, path, body) => {
  ...
  const doFetch = async (tk) => {
    ...
    logApiCall({
      client_id: client.id, channel: 'bvshop', method,
      endpoint: path.split('?')[0], res_status: resp.status,
      duration_ms: Date.now() - startAt,
    });
    return resp;
...
export const verifyToken = async (token, baseUrl = DEFAULT_BASE) => {
  ...
  try {
    ...
    logApiCall({
      client_id: null, channel: 'bvshop', method: 'GET',
      endpoint: '/api/v2/orders', res_status: resp.status,
      duration_ms: Date.now() - startAt,
    });
...
export default {
  ...
  fetchCustomer, searchBvCustomers,
