...
export const securityHeaders = (req, res, next) => {
  ...
  res.setHeader('X-Frame-Options', 'SAMEORIGIN');  // 允許自家 iframe overlay
  ...
  res.setHeader('Content-Security-Policy', [
    ...
    "style-src 'self' 'unsafe-inline' https://fonts.googleapis.com https://cdnjs.cloudflare.com https://stackpath.bootstrapcdn.com https://cdn.jsdelivr.net https://cdn.dhtmlx.com",
    ...
    "frame-ancestors 'self'",  // 允許自家 iframe overlay；外站仍擋
  ].join('; '));
...
// 暫時放寬以解 SW reload 風暴造成的 429
export const globalRateLimiter = createRateLimiter({
  windowMs: 60_000,
  max: 600,    // 從 60 → 600（10x），給 SW 風暴緩衝
