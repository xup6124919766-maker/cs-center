...
// lazy import audience_sync（避免循環依賴，rules.js → audience_sync.js → db.js 沒問題）
let _syncTagFn = null;
const getAudienceSync = async () => {
  if (!_syncTagFn) {
    try {
      const mod = await import('./audience_sync.js');
      _syncTagFn = mod.syncTagToLineAudience;
    } catch { _syncTagFn = null; }
  }
  return _syncTagFn;
};
let _decryptFn = null;
const getDecrypt = async () => {
  if (!_decryptFn) {
    try {
      ...
      _decryptFn = mod.decrypt;
    } catch { _decryptFn = null; }
  }
  return _decryptFn;
};
...
// ─── 執行 auto_tag ───
const executeAutoTag = async (rule, context) => {
  ...
  // ── LINE Audience Group 同步（非阻塞）──
  // 注意：廣播分眾用途，非 LINE OA 聊天標籤
  if (!added.length) return;
  const clientRow = db.prepare('SELECT audience_sync_enabled, line_access_token_enc FROM clients WHERE id = ?').get(client_id);
  if (!clientRow?.audience_sync_enabled || !clientRow?.line_access_token_enc) return;

  try {
    const syncFn = await getAudienceSync();
    const decryptFn = await getDecrypt();
    if (!syncFn || !decryptFn) return;
    ...
    const token = decryptFn(clientRow.line_access_token_enc);
    for (const tag of added) {
      await syncFn(client_id, tag, [cc.channel_user_id], 'add', token, customer_id);
    }
  } catch (e) {
    log.warn({ err: e.message, customer_id, client_id }, 'auto_tag audience sync 失敗（非阻塞）');
  }
};
