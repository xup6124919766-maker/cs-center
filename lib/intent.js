...
export const summarizeConversation = async (messages = []) => {
  ...
  const system = `你是客服摘要助手。讀完對話記錄後，回覆**純文字摘要**（不要 JSON、不要 markdown），格式：

📝 摘要：[3 句話：① 顧客問什麼 ② 處理到哪 ③ 待辦]
🏷️ 意圖：[詢價/下單/客訴/物流/售後/成分諮詢/其他]
✅ 待辦：[客服下一步要做什麼，30 字內]
...
繁體中文，整體 ≤ 200 字。`;
  ...
  // 純文字 parse — 從格式化文字抓 摘要 / 意圖 / 待辦
  const rawText = (result.text || '').trim().replace(/^```[\w]*\s*/i, '').replace(/```\s*$/i, '').trim();
  const sumMatch = rawText.match(/📝\s*摘要[:：]\s*([\s\S]*?)(?:\n[🏷✅]|$)/);
  const intMatch = rawText.match(/🏷️?\s*意圖[:：]\s*([^\n]+)/);
  const actMatch = rawText.match(/✅\s*待辦[:：]\s*([^\n]+)/);
  if (sumMatch) {
    return {
      summary: sumMatch[1].trim(),
      key_intent: intMatch?.[1]?.trim() || null,
      key_entities: { products: [], order_ids: [] },
      action_required: actMatch?.[1]?.trim() || null,
    };
  }
  // fallback：整段純文字當 summary
...
// ─── 舊解析路徑保留（向下相容，但 chat() 用 json_schema:false 後不會走到）───
const _legacyJsonParse = (result) => {
