...
export const summarizeConversation = async (messages = []) => {
  ...
  const system = `你是客服摘要助手。讀完對話記錄後，產出 JSON：
{"summary":"純文字摘要 ≤ 80 字，3 句：① 顧客問什麼 ② 處理到哪 ③ 待辦","key_intent":"詢價|下單|客訴|物流|售後|成分諮詢|其他","action_required":"客服下一步要做什麼，≤ 30 字"}

重要：summary 跟 action_required 都是純文字，不要再嵌套 JSON。只回 JSON 物件本身，不要 markdown code block。繁體中文。`;
