...
// 預設 draft 用 gemini-2.5-flash（主用）；voc 同；brand_coach 用最便宜的
const FEATURE_MODEL = {
  draft:       () => process.env.AI_DRAFT_MODEL       || 'gemini-2.5-flash',
  voc:         () => process.env.AI_VOC_MODEL         || 'gemini-2.5-flash',
  brand_coach: () => process.env.AI_BRAND_COACH_MODEL || 'gemini-2.5-flash',
};

// ─── Gemini fallback 模型序列 ───
const GEMINI_FALLBACK_MODELS = [
  'gemini-2.5-flash',       // 主用
  'gemini-2.5-flash-lite',  // 次用（更便宜）
  'gemini-1.5-flash',       // 備用（穩定）
];
...
const callGemini = async ({ messages, system, model, max_tokens = 1024, json_schema = null }) => {
  ...
  try {
    ...
    if (!r.ok) {
      const err = await r.text();
      // 改成 warn — Gemini 失敗會自動 fallback，不該汙染 ERROR log
      log.debug({ status: r.status, model, body: err.slice(0, 200) }, 'gemini API error (will fallback)');
      return { ok: false, error: `Gemini ${r.status}: ${err.slice(0, 200)}`, _retryable: r.status === 429 || r.status === 503 || r.status === 500 };
    }

    const data = await r.json();
    let text = data.candidates?.[0]?.content?.parts?.[0]?.text || '';
    log.debug({ provider: 'gemini' }, 'gemini OK');
    ...
    if (!json && json_schema) {
      // json_schema 模式但解不出 JSON → 視為失敗，讓上層 retry/fallback 接手
      // 降為 debug：每次 chain 失敗時 callGeminiWithRetry 會在最後打一個 warn
      log.debug({ text_len: text.length, text_end: text.slice(-100), finish_reason: data.candidates?.[0]?.finishReason }, 'gemini JSON parse failed');
      return {
        ok: false,
        error: `Gemini JSON parse failed (finish=${data.candidates?.[0]?.finishReason || 'unknown'})`,
        _retryable: true,
        text, // 留給上層 fallback 拼接資料
      };
    }
...
// ─── Gemini with retry（多模型 fallback + 網路抖動 retry）───
const callGeminiWithRetry = async (callOpts) => {
  if (!GEMINI_KEY()) return { ok: false, error: 'GEMINI_API_KEY 未設定' };

  // 組裝要嘗試的模型序列：指定模型優先，其餘依序 fallback
  const specifiedModel = callOpts.model;
  const models = specifiedModel
    ? [specifiedModel, ...GEMINI_FALLBACK_MODELS.filter(m => m !== specifiedModel)]
    : GEMINI_FALLBACK_MODELS;

  let lastErr = null;

  for (const model of models) {
    // 每個模型最多 retry 1 次（應對網路抖動）
    for (let attempt = 0; attempt < 2; attempt++) {
      if (attempt > 0) {
        await new Promise(r => setTimeout(r, 1000));
        log.info({ model, attempt }, 'gemini retry attempt');
      }
      try {
        const result = await callGemini({ ...callOpts, model });
        if (result.ok) return result;
        lastErr = result.error || 'unknown';
        // 同次呼叫只在 debug 留紀錄；最終失敗才升 warn
        log.debug({ model, attempt, error: lastErr }, 'gemini call failed, checking next');
        // 非配額/區域問題（如 JSON 解析錯誤）不值得 retry 同模型
        if (!lastErr.includes('429') && !lastErr.includes('503') && !lastErr.includes('500')) break;
      } catch (e) {
        lastErr = e.message;
        log.debug({ model, attempt, err: lastErr }, 'gemini call exception');
      }
    }
    log.debug({ model, error: lastErr }, 'gemini model exhausted, trying next model');
  }
  // 全部模型失敗才打 warn（一個 chain 只一行）
  log.warn({ tried: models, last_err: lastErr }, 'gemini all models exhausted');

  // 全部 Gemini 模型都失敗 → fallback 到 Anthropic Haiku
  if (ANTHROPIC_KEY()) {
    log.warn({ lastErr }, 'All Gemini models failed, falling back to Anthropic Haiku');
    try {
      const result = await callAnthropic({ ...callOpts, model: 'claude-haiku-4-5-20251001' });
      if (result.ok) return result;
...
export const chat = async (opts) => {
  ...
  // 支援 AI_DRAFT_MODEL / AI_VOC_MODEL / AI_BRAND_COACH_MODEL 環境變數覆寫
  ...
  // 模型前綴決定 provider — 不論 AI_PROVIDER env 設定為何
  const isGeminiModel = (callOpts.model || '').startsWith('gemini-');
  const isClaudeModel = (callOpts.model || '').startsWith('claude-');

  // 若請求 claude-* 但沒 Anthropic key → 把 model 拿掉，讓 Gemini 走預設模型
  // （否則會把無效 model 名字塞進 Gemini 端點，導致 404 / JSON parse fail）
  if (isClaudeModel && !ANTHROPIC_KEY()) {
    log.debug({ model: callOpts.model }, 'no Anthropic key — drop claude model, use Gemini default');
    delete callOpts.model;
  }

  if (isClaudeModel && ANTHROPIC_KEY()) {
    // claude-* 強制走 Anthropic；若 key 沒設則 fallback Gemini
    result = await callAnthropic(callOpts);
    if (!result.ok && GEMINI_KEY()) {
      log.warn({ err: result.error }, 'anthropic failed, fallback gemini (drop claude-only model)');
      const { model, ...rest } = callOpts;
      result = await callGeminiWithRetry(rest);
    }
  } else if (isGeminiModel || provider === 'gemini') {
    result = await callGeminiWithRetry(callOpts);
    ...
    if (ANTHROPIC_KEY()) {
      result = await callAnthropic(callOpts);
      if (!result.ok) {
        log.warn({ err: result.error }, 'anthropic failed, trying gemini with retry');
        if (GEMINI_KEY()) result = await callGeminiWithRetry(callOpts);
      }
    } else if (GEMINI_KEY()) {
      result = await callGeminiWithRetry(callOpts);
    } else {
