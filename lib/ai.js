  ...
  for (const model of models) {
    ...
    for (let attempt = 0; attempt < 2; attempt++) {
      ...
      try {
        ...
        log.warn({ model, attempt, error: lastErr }, 'gemini call failed, checking next');
        // 非配額/區域問題（如 JSON 解析錯誤）不值得 retry 同模型
        if (!lastErr.includes('429') && !lastErr.includes('503') && !lastErr.includes('500')) break;
      } catch (e) {
        lastErr = e.message;
        log.warn({ model, attempt, err: lastErr }, 'gemini call exception');
      }
    }
    log.warn({ model, error: lastErr }, 'gemini model exhausted, trying next model');
  }
...
export const chat = async (opts) => {
  ...
  // 若選到 gemini-* 模型，不論 AI_PROVIDER 設定一律走 Gemini 路徑（帶 retry fallback）
