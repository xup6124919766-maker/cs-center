...
export const generateDrafts = async ({ conversation_id, client_id, user_id = null, realtime = null }) => {
  ...
  // 3. 取 brand_dna；如有 active_voice 用 voices[name] 覆寫
  ...
  if (conv.active_voice && Array.isArray(brandDna.voices)) {
    const voice = brandDna.voices.find(v => v.name === conv.active_voice);
    if (voice) brandDna = { ...brandDna, ...voice }; // voice 欄位覆寫 base
  }
